/* ================================================================ */
/* session.js — LabSession Durable Object                          */
/*                                                                  */
/* Holds per-session state (customers, orders, ledger, chat         */
/* history) and orchestrates the OpenAI tool-calling loop.          */
/* ================================================================ */
...
export class LabSession {
  ...
  /* ---- Tool execution (intentionally vulnerable) ---- */
  ...
  executeTool(name, args) {
    switch (name) {
      ...
      case 'issue_refund': {
        ...
        // ⚠️  INTENTIONALLY VULNERABLE — no authorization check
        order.refunded = true;
  ...
  /* ---- Chat loop (calls OpenAI, handles tool calls) ---- */
  ...
  async fetch(request) {
    ...
    try {
      ...
      if (path === '/chat' && request.method === 'POST') {
        if (!this.initialized) {
          return Response.json(
            { error: 'Session not started. Call /lab/start first.' },
            { status: 400 },
          );
        }
      ...
      console.error('LabSession error:', err);
      return Response.json(
