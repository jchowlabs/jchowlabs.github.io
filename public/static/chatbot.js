...
(function () {
  ...
  var sessionStartTime = null;          // in-memory session start timestamp
  ...
  function startSession(greetingInstruction) {
    ...
    var tokenPromise = fetch(WORKER_URL + '/api/session', {
      method: 'POST',
  ...
  function handleResponseDone() {
    if (pendingNavigate) {
      ...
      pendingNavigate = null;
      // SPA navigation — session stays alive
      if (typeof window.__nextNavigate === 'function') {
  ...
  function openContact() {
    // SPA — contact modal is available globally on every page
